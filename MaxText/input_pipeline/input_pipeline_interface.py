...
def make_c4_train_iterator_and_tokenizer(config, mesh, add_bos, add_eos, process_indices):
  ...
  if config.use_high_res_data or config.mix_high_res_ratio > 0.0:
    read_config = tfds.ReadConfig(
      ...
      override_buffer_size=8,
    )
    get_datasets = (
        _tfds_data_processing.get_mixed_datasets
        if config.mix_high_res_ratio > 0.0
        else _tfds_data_processing.get_high_res_datasets
    )
...
def get_shaped_batch(config):
  ...
  shaped_batch = {}
  shaped_batch["inputs"] = jax.ShapeDtypeStruct(batch_shape, jnp.float32)
  # shaped_batch["inputs_position"] = jax.ShapeDtypeStruct(batch_shape, jnp.int32)
  # shaped_batch["inputs_segmentation"] = jax.ShapeDtypeStruct(batch_shape, jnp.int32)
  shaped_batch["targets"] = jax.ShapeDtypeStruct(batch_shape, jnp.float32)
