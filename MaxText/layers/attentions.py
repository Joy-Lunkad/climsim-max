...
class Attention(nn.Module):
  ...
  value_mask_init: NdInitializer = nd_dense_init(0.0, "fan_in", "normal")
  ...
  def __call__(
    ...
    if self.config.fused_qkv:
      ...
      B, L, H, D = value.shape # (4096, 1024, 4, 128)
      if self.config.use_value_masks:
        value_masks = self.param(
            "value_masks",
            nn.with_logical_partitioning(self.value_mask_init, ("embed", "heads", "kv")),
            (L, self.num_kv_heads, self.head_dim),
            self.weight_dtype,
            [0], 
            [1, 2],
        )
        value = jnp.einsum("blhd,lhd->blhd", value, value_masks)
