...
class SimpleDense(nn.Module):
  ...
  Attributes:
    ...
    kernel_axes: axes to partition the kernel params.
  ...
  def __call__(self, inputs: Array) -> Array:
    ...
    kernel_init = nn.initializers.variance_scaling(
      1.0, "fan_in", "truncated_normal", out_axis=0
      )
    ...
    output = jnp.dot(inputs, kernel, precision=None)

    if self.use_bias:
      bias_axes, bias_shape = self.kernel_axes[-1], self.out_features
      bias = self.param(
          "bias",
...
class DenseGeneral(nn.Module):
  ...
  def __call__(self, inputs: Array) -> Array:
    ...
    def compute_dot_general(inputs, kernel, axis, contract_ind):
      ...
      if self.use_feature_specific_mlp:
        return jnp.einsum('bld,ldh->blh', inputs, kernel, precision=None)
    ...
    if self.use_feature_specific_mlp:
      kernel_shape = (inputs.shape[1], *kernel_shape)
    ...
    if cfg.fused_mlp:
      ...
      for idx, act_fn in enumerate(self.activations):
        ...
        x = DenseGeneral(
            ...
            quant=self.quant,
            use_bias=self.use_bias,
            use_feature_specific_mlp=cfg.use_feature_specific_mlp,
        )(inputs)
