...
from max_utils import MIXED_NORM_COND
  ...
  def __call__(
    ...
    if cfg.logits_via_embedding:
      ...
      logits = linears.DenseGeneral(
          cfg.N_OUT if cfg.only_mlp else 1,
          weight_dtype=cfg.weight_dtype,
...
class Transformer(nn.Module):
  ...
  def setup(self):
    ...
    features = cfg.emb_dim
    if cfg.stack_inputs:
      features = self.config.stack_input_seq_length
    ...
    self.shared_embedding = Embed(
        ...
        # embedding_init=nn.initializers.normal(stddev=1.0),
        embedding_init=nn.initializers.zeros_init(),
        name="token_embedder",
        config=cfg,
    )
  ...
  def __call__(
    ...
    if self.config.stack_inputs or self.config.only_mlp:
      logits = jnp.transpose(logits, (0, 2, 1))
    
    y_pred = logits[:, -self.config.N_OUT:, 0]
    
    if self.config.using_mixed_y_norm:
      y_pred_sigmoid = jax.nn.sigmoid(y_pred)
      y_pred = jnp.where(MIXED_NORM_COND, y_pred_sigmoid, y_pred)
...
class MLP(nn.Module):
  ...
  def __call__(
    ...
    for i in range(cfg.num_decoder_layers):
      ...
      x = nn.tanh(x)
      x = nn.with_logical_constraint(x, ("activation_batch", "activation_mlp"))
      x = RMSNorm(
        name=f"mlp_norm_{i}",
        dtype=cfg.dtype,
        weight_dtype=cfg.weight_dtype,
    ...
    y_pred = nn.with_logical_constraint(y_pred, ("activation_batch", "activation_vocab"))
