...
def safe_norm_input(x):
    std = x_denorm_std_high_res_safe if x.shape[-1] == 556 else x_denorm_std
    mean = x_denorm_mean_high_res if x.shape[-1] == 556 else x_denorm_mean
    x = (x - mean) / std
    return x

def safe_denorm_input(x):
    std = x_denorm_std_high_res_safe if x.shape[-1] == 556 else x_denorm_std
    mean = x_denorm_mean_high_res if x.shape[-1] == 556 else x_denorm_mean
    x = x * std + mean
    return x
...
op_stds_idx_0 = np.where(op_stds < 1e-12)[0]
def change_y_std_0_to_mean(y):
    y[:, op_stds_idx_0] = op_means[op_stds_idx_0]
    return y
...
def keras_norm_output(y):
    ...
    y = tf.where(keras_std_y == 0, y, y_norm)
    return y
...
MIXED_NORM_COND = np.logical_and(cond1, cond2)

def mixed_norm_output(y):
  keras_y_norm = keras_norm_output(y)
  min_max_y_norm = min_max_norm_output(y)
  y =tf.where(MIXED_NORM_COND, min_max_y_norm, keras_y_norm)
  return y

def mixed_denorm_output(y):
  keras_y_denorm = keras_denorm_output(y)
  min_max_y_denorm = min_max_denorm_output(y)
  y = np.where(MIXED_NORM_COND, min_max_y_denorm, keras_y_denorm)
  return y


def which_norm_input(x):
  return keras_norm_input(x)

def which_denorm_input(x):
  return keras_denorm_input(x)

def which_norm_output(y):
  return keras_norm_output(y)

def which_denorm_output(y):
  return keras_denorm_output(y)
