...
def get_functional_forward_with_signature(forward, mesh, state_mesh_annotations, model, config):
  """Get the shardings (both state and data) for eval_step"""
  functional_forward = functools.partial(forward, model, config)
  functional_forward.__name__ = "forward"
  data_pspec = P(*config.data_sharding)
  state_mesh_shardings = jax.tree_util.tree_map(lambda p: jax.sharding.NamedSharding(mesh, p), state_mesh_annotations)
  data_sharding = jax.tree_util.tree_map(lambda p: jax.sharding.NamedSharding(mesh, p), data_pspec)
  in_shardings = (state_mesh_shardings, data_sharding, None)  # State, batch, rng
  ...
  static_argnums = ()
  donate_argnums = ()  
  return functional_forward, in_shardings, out_shardings, static_argnums, donate_argnums
