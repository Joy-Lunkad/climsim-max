...
class _HyperParameters:
    ...
    def user_init(raw_keys):
        ...
        raw_keys["full_low_res_version"] = raw_keys.get("full_low_res_version", "2.0.0")
        USE_FULL_DATA = (
            raw_keys["use_full_low_res_data"] or raw_keys["use_high_res_data"]
        )

        raw_keys["N_IN"] = 556 if USE_FULL_DATA else 490
        raw_keys["N_OUT"] = 368 if USE_FULL_DATA else 308
        raw_keys["grid_size"] = 21600 if raw_keys["use_high_res_data"] else 384
        ...
        raw_keys["train_steps_per_epoch"] = (
            train_samples_per_epoch // raw_keys["global_batch_size_to_train_on"]
        )

        raw_keys["eval_steps"] = (
            val_samples_per_epoch // raw_keys["global_batch_size_to_train_on"]
        )

        if raw_keys["full_low_res_version"] == "2.0.0":
            raw_keys["train_steps_per_epoch"] = train_samples_per_epoch // (
                raw_keys["global_batch_size_to_train_on"] * raw_keys["grid_size"]
            )
            raw_keys["eval_steps"] = val_samples_per_epoch // (
                raw_keys["global_batch_size_to_train_on"] * raw_keys["grid_size"]
            )
            raw_keys["IS_GRID_DATA"] = True

        epochs = raw_keys.get("num_epochs", None)
        if epochs:
            print("Inheriting steps from num_epochs")
            raw_keys["steps"] = int(epochs * raw_keys["train_steps_per_epoch"])
        ...
        if n_evals_every_epoch:
            raw_keys["eval_interval"] = int(
                raw_keys["train_steps_per_epoch"] * (1 / n_evals_every_epoch)
            )
        ...
        if n_checkpoints_every_epoch:
            raw_keys["checkpoint_period"] = int(
                raw_keys["train_steps_per_epoch"] * (1 / n_checkpoints_every_epoch)
            )
        ...
        raw_keys["use_feature_specific_mlp"] = raw_keys.get(
            "use_feature_specific_mlp", False
        )
        ...
        raw_keys["shuffle_buffer_multiplier"] = raw_keys.get(
