...
class _HyperParameters:
  ...
  def user_init(raw_keys):
    ...
    raw_keys["use_full_low_res_data"] = raw_keys.get("use_full_low_res_data", False)
    raw_keys["use_high_res_data"] = raw_keys.get("use_high_res_data", False)
    raw_keys["mix_high_res_ratio"] = raw_keys.get("mix_high_res_ratio", 0.0)    
    USE_FULL_DATA = raw_keys["use_full_low_res_data"] or raw_keys["use_high_res_data"]
    
    raw_keys["N_IN"] = 556 if USE_FULL_DATA else 490
    raw_keys["N_OUT"] = 368 if USE_FULL_DATA else 308
    
    if USE_FULL_DATA:
      raw_keys["max_target_length"] = 556
    
    
    train_samples_per_epoch = 10_000_000
    if raw_keys["use_high_res_data"]:
      train_samples_per_epoch = 563_176_800
    
    val_samples_per_epoch = 500_000
    test_samples = 625_000
    
    train_steps_per_epoch = train_samples_per_epoch // raw_keys["global_batch_size_to_train_on"]
    raw_keys["train_steps_per_epoch"] = train_steps_per_epoch
    
    eval_steps = val_samples_per_epoch // raw_keys["global_batch_size_to_train_on"]
    raw_keys["eval_steps"] = eval_steps
    
    epochs = raw_keys.get("num_epochs", None)
    if epochs:
      print("Inheriting steps from num_epochs")
      raw_keys["steps"] = int(epochs * train_steps_per_epoch)  
    ...
    print("Total Training Steps -> ", raw_keys["steps"])



    n_evals_every_epoch = raw_keys.get("n_evals_every_epoch", None)
    if raw_keys["use_high_res_data"]:
      n_evals_every_epoch *= 50
    if n_evals_every_epoch:
      raw_keys["eval_interval"] = int(train_steps_per_epoch * (1/n_evals_every_epoch))
    print("eval_interval -> ", raw_keys["eval_interval"])

    n_checkpoints_every_epoch = raw_keys.get("n_checkpoints_every_epoch", None)
    if raw_keys["use_high_res_data"]:
      n_checkpoints_every_epoch *= 50
    if n_checkpoints_every_epoch:
      raw_keys["checkpoint_period"] = int(train_steps_per_epoch * (1/n_checkpoints_every_epoch))
    print("checkpoint_period -> ", raw_keys["checkpoint_period"])
    ...
    raw_keys["use_value_masks"] = raw_keys.get("use_value_masks", False)
    
    
    raw_keys["only_mlp"] = raw_keys.get("only_mlp", False)
    raw_keys["stack_inputs"] = raw_keys.get("stack_inputs", False)
    raw_keys["use_feature_specific_mlp"] = raw_keys.get("use_feature_specific_mlp", False)
    raw_keys["stack_input_seq_length"] = raw_keys.get("stack_input_seq_length", 1)
    
    raw_keys["n_ds_per_split"] = raw_keys.get("n_ds_per_split", 8)
    raw_keys["n_repeat_ds_per_epoch"] = raw_keys.get("n_repeat_ds_per_epoch", 1)
    raw_keys["shuffle_buffer_multiplier"] = raw_keys.get("shuffle_buffer_multiplier", 1)
    raw_keys["use_submission_trick"] = raw_keys.get("use_submission_trick", False)
    raw_keys["full_low_res_version"] = raw_keys.get("full_low_res_version", "3.0.0")
    
    
    raw_keys["normalize"] = False
    if (raw_keys["use_full_low_res_data"] and raw_keys["full_low_res_version"] in [
      "3.0.0",
      "6.0.0",
      "7.0.0",
    ]) or raw_keys["use_high_res_data"]:
      raw_keys["normalize"] = True
    
    raw_keys["using_mixed_y_norm"] = raw_keys.get("using_mixed_y_norm", False)
